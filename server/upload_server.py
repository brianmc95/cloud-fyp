...
class SimpleHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        success = False
        if "/account/openstack/" in self.path:
            length = int(self.headers["content-length"])
            post_body = self.rfile.read(length).decode("utf-8", "ignore")

            result = self.dm.add_openstack_account(post_body, "openstack")
            if result:
                self.send_response(200)
                self.end_headers()
                success = True

        elif "/account/aws/" in self.path:
            length = int(self.headers["content-length"])
            post_body = self.rfile.read(length).decode("utf-8", "ignore")
            account = json.loads(post_body)
            result = self.dm.add_account(account, "aws")
            if result:
                self.send_response(200)
                self.end_headers()
                success = True
...
def main():
    ...
    if args.ssl:
        sec = True
        httpd.socket = ssl.wrap_socket(httpd.socket, certfile=''.format(args.ssl), server_side=True)

    print("Serving HTTP on {} port: {} with security {} ...".format(args.ip, args.port, sec))
    httpd.serve_forever()
